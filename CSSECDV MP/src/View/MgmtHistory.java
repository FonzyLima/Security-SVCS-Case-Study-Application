...
//      LOAD CONTENTS
        if(CurrentUserSession.role==4){
            ...
            ArrayList<History> history = sqlite.getHistoryByUser(CurrentUserSession.currentUser);
            for(int nCtr = 0; nCtr < history.size(); nCtr++){
                Product product = sqlite.getProduct(history.get(nCtr).getName());
                tableModel.addRow(new Object[]{
                    history.get(nCtr).getUsername(), 
...
//          LOAD CONTENTS
            if(CurrentUserSession.role == 2){
                ...
                if(history.isEmpty()){
                    history = sqlite.getHistoryByProduct(searchFld.getText().toLowerCase());
                }
                for(int nCtr = 0; nCtr < history.size(); nCtr++){
                    if(searchFld.getText().contains(history.get(nCtr).getUsername()) || 
                       history.get(nCtr).getUsername().contains(searchFld.getText()) || 
